 ...
 * @description Advanced HTML5 Canvas rendering engine for the infinite whiteboard.
 * This class handles:
 * - Coordinate systems (Screen vs World space)
 * - Infinite panning and zooming
 * - Rendering of primitives (Shapes, Lines, Text, Images, PDF previews)
 * - Interaction logic (Selection, Dragging, Resizing)
...
class FluxWhiteboard {
    ...
    constructor(canvasId) {
        ...
        /** @type {Object} Engine configuration parameters */
        this.config = {
            gridEnabled: true,
            dotGap: 40,           // Distance between grid dots in world space
            dotRadius: 1.2,
            minScale: 0.1,        // Max zoom out
            maxScale: 10,         // Max zoom in
        ...
        this.interaction = {
            ...
            // Proportional scaling data snapshot
            initialWidth: 0, 
            initialHeight: 0, 
            aspectRatio: 1, 
            initialFontSize: 16
    ...
    /**
     * @method init
     * @description Sets up event listeners for mouse, touch, and resize events.
     */
    init() {
        ...
        // Touch Events (Passive false to allow preventing default scrolling)
     ...
     * Removes non-serializable objects (like Image objects) before returning.
     * @returns {Object} Clean serializable state object.
     */
    getState() {
        const cleanElements = this.elements.map(el => {
            const copy = { ...el };
            // Remove cached DOM image objects to allow JSON stringification
    ...
    /**
     * @method loadState
     * @description Loads a previously saved state and re-renders the board.
     * @param {Object} state - The state object loaded from JSON.
     */
    loadState(state) {
        if (!state) return;
        this.elements = state.elements || [];
        // Default view centers the board if missing
     ...
     * @description Normalizes pointer coordinates relative to the canvas DOM element.
    ...
    // --- COORDINATE SYSTEMS ---
    // Screen: Pixels on the monitor (0,0 is top-left of canvas)
    // World: Infinite coordinate system where elements live
    ...
    /**
     * @method saveHistory
     * @description Snapshots the current elements array to the undo stack.
     */
    saveHistory() {
    ...
    // These methods create specific element types and push them to the array.
    ...
    /**
     * @method addPDF
     * @description Creates a PDF element wrapper. 
     * Note: This does NOT render the PDF pages on the board directly to avoid memory issues.
     * It renders a preview card, and opens the full reader on double click.
     */
    addPDF(fileName, fileUrl) {
        ...
        const newPDF = {
            ...
            height: 90, 
            renderedImage: null,
            annotations: [] // Stores highlights/drawings relative to the PDF pages
        };
    ...
    duplicateSelected() {
        ...
        this.interaction.selectedElements.forEach(el => {
            // Deep copy via JSON to break references
            const clone = JSON.parse(JSON.stringify(el)); 
            clone.id = Date.now() + Math.random();
            
            const offset = 20 / this.view.scale;
            
            // Offset positions so duplicates don't stack perfectly on top
            if (clone.type === 'line') { 
     ...
     * @description Switches auto-colored elements between light/dark values.
     */
    ...
    handleWheel(e) {
        ...
        // Control + Wheel = Zoom
        if (e.ctrlKey) {
            this.applyZoom(1 - e.deltaY * this.config.zoomSensitivity, e.clientX, e.clientY);
        } else {
            // Regular Wheel = Pan
    ...
    applyZoom(factor, x, y) {
        ...
        // Re-calculate offset so that mouseWorld stays under pointer
    ...
    handleMouseDown(e) {
        ...
        // 1. PANNING (Middle click or Space+Drag or Pan Tool)
        const clientX = e.touches ? e.touches[0].clientX : e.clientX;
        const clientY = e.touches ? e.touches[0].clientY : e.clientY;
        ...
        for (const el of this.interaction.selectedElements) {
            ...
            for (let i = 0; i < handles.length; i++) {
                const h = handles[i];
                // Check distance to handle in screen pixels
                if (Math.hypot(mouse.x - h.x, mouse.y - h.y) * this.view.scale < this.config.handleHitThreshold) {
                    ...
                    // Store initial state for proportional resizing
        ...
        if (tool === 'select') {
            ...
            for (let i = this.elements.length - 1; i >= 0; i--) {
                ...
                if (this.isPointInElement(mouse, el)) {
                    ...
                    // PDF Preview Button Specific Detection
                    if (el.type === 'pdf') {
            ...
            // If background clicked, start marquee selection
            if (!hitFound) { 
                this.interaction.selectedElements = []; 
    ...
    handleMouseMove(e) {
        ...
        // 1. CURSOR HOVER LOGIC (PDF Button)
        let cursorSet = false;
    ...
    handleMouseUp() {
        ...
        this.interaction.isDrawingPath = false; 
    ...
    getElementHandles(el) {
        ...
        // PDF has handles only on corners (resize constrain preferred)
        if (el.type === 'pdf') {
        ...
        // Box elements (Text, Shape, Image) have 8 handles
    ...
    resizeElement(el, handleIdx, mouse) {
        if (el.renderedImage) el.renderedImage = null; // Invalidate cache
        ...
        if (el.type === 'shape' || el.type === 'text' || el.type === 'image' || el.type === 'pdf') {
            ...
            // 1. Calculate new dimensions based on handle direction
            switch(handleIdx) {
            ...
            // 2. Enforce Proportional scaling for Images/PDFs on corners
            if (isCorner && el.type !== 'text') {
                if (el.type === 'image' || el.type === 'pdf') {
                    ...
                    // Shapes resize proportionally too on corners
                    if (el.width / ratio > el.height) el.height = el.width / ratio;
                    else el.width = el.height * ratio;
                }
            }

            // 3. Min size constraints
            if (el.width < minSize) { 
                el.width = minSize; 
            ...
            // 4. Adjust Position (Anchor logic)
    ...
    /**
     * @method isPointInElement
     * @description Hit testing logic for selection.
     */
    isPointInElement(p, el) {
        if (el.type === 'line' || el.type === 'pen') {
            // Calculate distance to segment/polyline
    ...
    finalizeSelection() {
        ...
        this.interaction.selectedElements = this.elements.filter(el => {
            ...
            if (el.type === 'shape' || el.type === 'text' || el.type === 'image' || el.type === 'pdf') {
                // Strict containment for box elements
                return el.x >= x1 && el.x + el.width <= x2 && el.y >= y1 && el.y + el.height <= y2;
            }
            return false;
        });
    }
    ...
    handleTouchMove(e) {
        if (e.touches.length === 2 && this.interaction.isPanning) {
            ...
            // Calculate zoom factor based on pinch spread
    ...
    drawInfiniteGrid() {
        ...
        // Hide grid if too dense to prevent moiré patterns
        if (gap < 8) return;
        
        const sX = (this.view.offsetX % gap) - gap;
        const sY = (this.view.offsetY % gap) - gap;
        
        this.ctx.beginPath();
        for (let x = sX; x < window.innerWidth + gap; x += gap) {
            for (let y = sY; y < window.innerHeight + gap; y += gap) {
                this.ctx.moveTo(x, y); 
     ...
     * @description Generates an SVG representation of the PDF Preview Card.
     * Uses foreignObject to embed HTML styled via inline CSS.
     */
    renderPDFToImage(el) {
     ...
     * @description Converts Markdown + LaTeX content into an SVG image.
     * Uses `marked` library for MD and `katex` for math.
     */
    renderMarkdownToImage(el) {
        if (el.isRendering) return;
        el.isRendering = true;
        
        let htmlContent = window.marked ? window.marked.parse(el.content) : `<p>${el.content}</p>`;
    ...
    drawSelectionBox(sPos, sW, sH, el) {
    ...
    drawSelectionMarquee() {
        ...
        const b = this.interaction.selectionBox; 
        const p1=this.worldToScreen(b.startX,b.startY),p2=this.worldToScreen(b.currentX,b.currentY);
        this.ctx.save(); 
        this.ctx.setLineDash([5,5]); 
        this.ctx.strokeStyle=getComputedStyle(document.body).getPropertyValue('--accent-color');
        this.ctx.lineWidth=1; 
    ...
    drawHandle(x,y,c) { 
        ...
        this.ctx.fillStyle=getComputedStyle(document.body).getPropertyValue('--surface-color'); 
        this.ctx.fill(); 
